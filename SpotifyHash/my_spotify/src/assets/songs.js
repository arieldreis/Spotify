const musicasTheWeeknd = [
  { id: 1, title: "Blinding Lights", artista: "The Weeknd", duracao: "3:20" },
  { id: 2, title: "Save Your Tears", artista: "The Weeknd", duracao: "3:35" },
  { id: 3, title: "Starboy", artista: "The Weeknd", duracao: "3:50" },
  { id: 4, title: "The Hills", artista: "The Weeknd", duracao: "4:02" },
  { id: 5, title: "Can't Feel My Face", artista: "The Weeknd", duracao: "3:35" },
  { id: 6, title: "Earned It", artista: "The Weeknd", duracao: "4:10" },
  { id: 7, title: "In Your Eyes", artista: "The Weeknd", duracao: "3:58" },
  { id: 8, title: "I Feel It Coming", artista: "The Weeknd", duracao: "4:29" },
  { id: 9, title: "Out of Time", artista: "The Weeknd", duracao: "3:34" },
  { id: 10, title: "Take My Breath", artista: "The Weeknd", duracao: "3:40" }
];

const musicasTravisScott = [
  { id: 11, title: "SICKO MODE", artista: "Travis Scott", duracao: "5:12" },
  { id: 12, title: "goosebumps", artista: "Travis Scott", duracao: "4:03" },
  { id: 13, title: "HIGHEST IN THE ROOM", artista: "Travis Scott", duracao: "2:55" },
  { id: 14, title: "MELTDOWN", artista: "Travis Scott", duracao: "4:05" },
  { id: 15, title: "FE!N", artista: "Travis Scott", duracao: "3:11" },
  { id: 16, title: "STARGAZING", artista: "Travis Scott", duracao: "4:30" },
  { id: 17, title: "BUTTERFLY EFFECT", artista: "Travis Scott", duracao: "3:10" },
  { id: 18, title: "Antidote", artista: "Travis Scott", duracao: "4:22" },
  { id: 19, title: "STOP TRYING TO BE GOD", artista: "Travis Scott", duracao: "5:38" },
  { id: 20, title: "90210", artista: "Travis Scott", duracao: "5:40" }
];

const musicasTaylorSwift = [
  { id: 21, title: "Cruel Summer", artista: "Taylor Swift", duracao: "2:58" },
  { id: 22, title: "Anti-Hero", artista: "Taylor Swift", duracao: "3:21" },
  { id: 23, title: "Shake It Off", artista: "Taylor Swift", duracao: "3:39" },
  { id: 24, title: "Love Story", artista: "Taylor Swift", duracao: "3:55" },
  { id: 25, title: "You Belong With Me", artista: "Taylor Swift", duracao: "3:52" },
  { id: 26, title: "Blank Space", artista: "Taylor Swift", duracao: "3:51" },
  { id: 27, title: "Style", artista: "Taylor Swift", duracao: "3:51" },
  { id: 28, title: "Enchanted", artista: "Taylor Swift", duracao: "5:52" },
  { id: 29, title: "The Archer", artista: "Taylor Swift", duracao: "3:31" },
  { id: 30, title: "Willow", artista: "Taylor Swift", duracao: "3:34" }
];

const musicasSZA = [
  { id: 31, title: "Kill Bill", artista: "SZA", duracao: "2:33" },
  { id: 32, title: "Good Days", artista: "SZA", duracao: "4:39" },
  { id: 33, title: "Snooze", artista: "SZA", duracao: "3:22" },
  { id: 34, title: "I Hate U", artista: "SZA", duracao: "2:54" },
  { id: 35, title: "Love Galore", artista: "SZA", duracao: "4:35" },
  { id: 36, title: "Nobody Gets Me", artista: "SZA", duracao: "3:00" },
  { id: 37, title: "The Weekend", artista: "SZA", duracao: "4:32" },
  { id: 38, title: "Shirt", artista: "SZA", duracao: "3:02" },
  { id: 39, title: "Low", artista: "SZA", duracao: "3:00" },
  { id: 40, title: "Seek & Destroy", artista: "SZA", duracao: "3:25" }
];

const musicasShawnMendes = [
  { id: 41, title: "Señorita", artista: "Shawn Mendes", duracao: "3:11" },
  { id: 42, title: "Stitches", artista: "Shawn Mendes", duracao: "3:27" },
  { id: 43, title: "There's Nothing Holdin' Me Back", artista: "Shawn Mendes", duracao: "3:20" },
  { id: 44, title: "Treat You Better", artista: "Shawn Mendes", duracao: "3:07" },
  { id: 45, title: "If I Can't Have You", artista: "Shawn Mendes", duracao: "3:11" },
  { id: 46, title: "In My Blood", artista: "Shawn Mendes", duracao: "3:31" },
  { id: 47, title: "Wonder", artista: "Shawn Mendes", duracao: "2:52" },
  { id: 48, title: "Mercy", artista: "Shawn Mendes", duracao: "3:28" },
  { id: 49, title: "Lost in Japan", artista: "Shawn Mendes", duracao: "3:21" },
  { id: 50, title: "Nervous", artista: "Shawn Mendes", duracao: "2:44" }
];

const musicasSeuJorge = [
  { id: 51, title: "Burguesinha", artista: "Seu Jorge", duracao: "3:46" },
  { id: 52, title: "Amiga da Minha Mulher", artista: "Seu Jorge", duracao: "3:58" },
  { id: 53, title: "Carolina", artista: "Seu Jorge", duracao: "3:42" },
  { id: 54, title: "Tive Razão", artista: "Seu Jorge", duracao: "3:50" },
  { id: 55, title: "Quem Não Quer Sou Eu", artista: "Seu Jorge", duracao: "4:17" },
  { id: 56, title: "Mina do Condomínio", artista: "Seu Jorge", duracao: "3:41" },
  { id: 57, title: "São Gonça", artista: "Seu Jorge", duracao: "3:38" },
  { id: 58, title: "Zé do Caroço", artista: "Seu Jorge", duracao: "4:10" },
  { id: 59, title: "Dois Barcos", artista: "Seu Jorge", duracao: "4:25" },
  { id: 60, title: "Life on Mars?", artista: "Seu Jorge", duracao: "3:41" }
];

const musicasSeanPaul = [
  { id: 61, title: "Temperature", artista: "Sean Paul", duracao: "3:38" },
  { id: 62, title: "Get Busy", artista: "Sean Paul", duracao: "3:31" },
  { id: 63, title: "No Lie", artista: "Sean Paul", duracao: "3:43" },
  { id: 64, title: "She Doesn't Mind", artista: "Sean Paul", duracao: "3:47" },
  { id: 65, title: "I'm Still in Love with You", artista: "Sean Paul", duracao: "4:36" },
  { id: 66, title: "Like Glue", artista: "Sean Paul", duracao: "3:53" },
  { id: 67, title: "Gimme the Light", artista: "Sean Paul", duracao: "3:46" },
  { id: 68, title: "Rockabye", artista: "Sean Paul", duracao: "4:11" },
  { id: 69, title: "Give It Up to Me", artista: "Sean Paul", duracao: "4:19" },
  { id: 70, title: "Baby Boy", artista: "Sean Paul", duracao: "4:07" }
];

const musicasRomeoSantos = [
  { id: 71, title: "Propuesta Indecente", artista: "Romeo Santos", duracao: "3:50" },
  { id: 72, title: "Eres Mía", artista: "Romeo Santos", duracao: "3:36" },
  { id: 73, title: "Hilito", artista: "Romeo Santos", duracao: "4:10" },
  { id: 74, title: "Odio", artista: "Romeo Santos", duracao: "3:44" },
  { id: 75, title: "Imitadora", artista: "Romeo Santos", duracao: "3:55" },
  { id: 76, title: "Bella y Sensual", artista: "Romeo Santos", duracao: "3:52" },
  { id: 77, title: "Necio", artista: "Romeo Santos", duracao: "4:22" },
  { id: 78, title: "Sobredosis", artista: "Romeo Santos", duracao: "4:10" },
  { id: 79, title: "Cancioncitas de Amor", artista: "Romeo Santos", duracao: "3:45" },
  { id: 80, title: "Mi Santa", artista: "Romeo Santos", duracao: "4:02" }
];

const musicasPostMalone = [
  { id: 81, title: "Circles", artista: "Post Malone", duracao: "3:35" },
  { id: 82, title: "Sunflower", artista: "Post Malone", duracao: "2:38" },
  { id: 83, title: "rockstar", artista: "Post Malone", duracao: "3:38" },
  { id: 84, title: "Congratulations", artista: "Post Malone", duracao: "3:40" },
  { id: 85, title: "Wow.", artista: "Post Malone", duracao: "2:29" },
  { id: 86, title: "Goodbyes", artista: "Post Malone", duracao: "2:54" },
  { id: 87, title: "I Like You", artista: "Post Malone", duracao: "3:11" },
  { id: 88, title: "Better Now", artista: "Post Malone", duracao: "3:51" },
  { id: 89, title: "White Iverson", artista: "Post Malone", duracao: "4:17" },
  { id: 90, title: "Mourning", artista: "Post Malone", duracao: "3:21" }
];

const musicasPitbull = [
  { id: 91, title: "Timber", artista: "Pitbull", duracao: "3:24" },
  { id: 92, title: "Give Me Everything", artista: "Pitbull", duracao: "4:12" },
  { id: 93, title: "Hotel Room Service", artista: "Pitbull", duracao: "3:58" },
  { id: 94, title: "Fireball", artista: "Pitbull", duracao: "3:24" },
  { id: 95, title: "Don't Stop the Party", artista: "Pitbull", duracao: "3:26" },
  { id: 96, title: "International Love", artista: "Pitbull", duracao: "3:47" },
  { id: 97, title: "Time of Our Lives", artista: "Pitbull", duracao: "3:49" },
  { id: 98, title: "Rain Over Me", artista: "Pitbull", duracao: "3:51" },
  { id: 99, title: "I Know You Want Me", artista: "Pitbull", duracao: "3:57" },
  { id: 100, title: "Hey Baby (Drop It to the Floor)", artista: "Pitbull", duracao: "3:24" }
];

const musicasOneRepublic = [
  { id: 101, title: "Counting Stars", artista: "OneRepublic", duracao: "4:17" },
  { id: 102, title: "Apologize", artista: "OneRepublic", duracao: "3:28" },
  { id: 103, title: "Secrets", artista: "OneRepublic", duracao: "3:44" },
  { id: 104, title: "Love Runs Out", artista: "OneRepublic", duracao: "3:44" },
  { id: 105, title: "I Lived", artista: "OneRepublic", duracao: "3:54" },
  { id: 106, title: "If I Lose Myself", artista: "OneRepublic", duracao: "4:01" },
  { id: 107, title: "Stop and Stare", artista: "OneRepublic", duracao: "4:04" },
  { id: 108, title: "Rescue Me", artista: "OneRepublic", duracao: "2:39" },
  { id: 109, title: "Run", artista: "OneRepublic", duracao: "2:49" },
  { id: 110, title: "Sunshine", artista: "OneRepublic", duracao: "2:43" }
];

const musicasOneDirection = [
  { id: 111, title: "What Makes You Beautiful", artista: "One Direction", duracao: "3:20" },
  { id: 112, title: "Story of My Life", artista: "One Direction", duracao: "4:05" },
  { id: 113, title: "Drag Me Down", artista: "One Direction", duracao: "3:12" },
  { id: 114, title: "Night Changes", artista: "One Direction", duracao: "3:46" },
  { id: 115, title: "Steal My Girl", artista: "One Direction", duracao: "3:48" },
  { id: 116, title: "Little Things", artista: "One Direction", duracao: "3:37" },
  { id: 117, title: "Best Song Ever", artista: "One Direction", duracao: "3:22" },
  { id: 118, title: "One Thing", artista: "One Direction", duracao: "3:17" },
  { id: 119, title: "Live While We're Young", artista: "One Direction", duracao: "3:20" },
  { id: 120, title: "Perfect", artista: "One Direction", duracao: "3:50" }
];

const michaelJacksonSongs = [
  { id: 121, title: "Billie Jean", artist: "Michael Jackson", duration: "4:54" },
  { id: 122, title: "Thriller", artist: "Michael Jackson", duration: "5:57" },
  { id: 123, title: "Beat It", artist: "Michael Jackson", duration: "4:18" },
  { id: 124, title: "Smooth Criminal", artist: "Michael Jackson", duration: "4:17" },
  { id: 125, title: "Bad", artist: "Michael Jackson", duration: "4:07" },
  { id: 126, title: "Black or White", artist: "Michael Jackson", duration: "4:16" },
  { id: 127, title: "Man in the Mirror", artist: "Michael Jackson", duration: "5:19" },
  { id: 128, title: "The Way You Make Me Feel", artist: "Michael Jackson", duration: "4:58" },
  { id: 129, title: "Remember the Time", artist: "Michael Jackson", duration: "4:00" },
  { id: 130, title: "Don't Stop 'Til You Get Enough", artist: "Michael Jackson", duration: "6:05" },
];

const queenSongs = [
  { id: 131, title: "Bohemian Rhapsody", artist: "Queen", duration: "5:55" },
  { id: 132, title: "Don't Stop Me Now", artist: "Queen", duration: "3:29" },
  { id: 133, title: "We Will Rock You", artist: "Queen", duration: "2:02" },
  { id: 134, title: "Another One Bites The Dust", artist: "Queen", duration: "3:35" },
  { id: 135, title: "Somebody to Love", artist: "Queen", duration: "4:56" },
  { id: 136, title: "Crazy Little Thing Called Love", artist: "Queen", duration: "2:42" },
  { id: 137, title: "We Are The Champions", artist: "Queen", duration: "3:01" },
  { id: 138, title: "Under Pressure", artist: "Queen & David Bowie", duration: "4:08" },
  { id: 139, title: "Killer Queen", artist: "Queen", duration: "3:00" },
  { id: 140, title: "Radio Ga Ga", artist: "Queen", duration: "5:48" },
];

const justinBieberSongs = [
  { id: 141, title: "Sorry", artist: "Justin Bieber", duration: "3:20" },
  { id: 142, title: "Love Yourself", artist: "Justin Bieber", duration: "3:54" },
  { id: 143, title: "Peaches", artist: "Justin Bieber", duration: "3:18" },
  { id: 144, title: "Baby", artist: "Justin Bieber", duration: "3:36" },
  { id: 145, title: "What Do You Mean?", artist: "Justin Bieber", duration: "3:25" },
  { id: 146, title: "Where Are Ü Now", artist: "Justin Bieber & Jack Ü", duration: "4:10" },
  { id: 147, title: "Yummy", artist: "Justin Bieber", duration: "3:30" },
  { id: 148, title: "Intentions", artist: "Justin Bieber ft. Quavo", duration: "3:32" },
  { id: 149, title: "Never Say Never", artist: "Justin Bieber", duration: "3:39" },
  { id: 150, title: "One Time", artist: "Justin Bieber", duration: "3:31" },
];

const menudoSongs = [
  { id: 151, title: "Não Se Reprima", artist: "Menudo", duration: "3:38" },
  { id: 152, title: "Quero Ser", artist: "Menudo", duration: "3:44" },
  { id: 153, title: "Diga Sim", artist: "Menudo", duration: "3:46" },
  { id: 154, title: "Rock da Geração", artist: "Menudo", duration: "3:28" },
  { id: 155, title: "Se Tu Não Estás", artist: "Menudo", duration: "4:10" },
  { id: 156, title: "Súbete a Mi Moto", artist: "Menudo", duration: "3:15" },
  { id: 157, title: "Claridad", artist: "Menudo", duration: "3:50" },
  { id: 158, title: "Si Tú No Estás", artist: "Menudo", duration: "4:08" },
  { id: 159, title: "Chicle de Amor", artist: "Menudo", duration: "3:30" },
  { id: 160, title: "Ella-a-a", artist: "Menudo", duration: "3:45" },
];

const mcCabelinhoSongs = [
  { id: 161, title: "Coração de Gelo", artist: "MC Cabelinho", duration: "3:02" },
  { id: 162, title: "Você Gosta Assim", artist: "MC Cabelinho", duration: "2:58" },
  { id: 163, title: "Barulho do Foguete", artist: "MC Cabelinho", duration: "2:32" },
  { id: 164, title: "X1", artist: "MC Cabelinho ft. Xamã", duration: "2:56" },
  { id: 165, title: "Minha Cura", artist: "MC Cabelinho", duration: "3:20" },
  { id: 166, title: "Fogo e Gasolina", artist: "MC Cabelinho", duration: "2:40" },
  { id: 167, title: "Modo Avião", artist: "MC Cabelinho", duration: "2:50" },
  { id: 168, title: "Aonde É o Baile", artist: "MC Cabelinho", duration: "2:48" },
  { id: 169, title: "Ela É do Tipo", artist: "MC Cabelinho", duration: "2:46" },
  { id: 170, title: "Pepeka Braba", artist: "MC Cabelinho", duration: "2:55" },
];

const matueSongs = [
  { id: 171, title: "Quer Voar", artist: "Matuê", duration: "3:13" },
  { id: 172, title: "Máquina do Tempo", artist: "Matuê", duration: "3:12" },
  { id: 173, title: "777-666", artist: "Matuê", duration: "2:36" },
  { id: 174, title: "Kenny G", artist: "Matuê", duration: "2:52" },
  { id: 175, title: "Groupies", artist: "Matuê", duration: "3:10" },
  { id: 176, title: "Anos Luz", artist: "Matuê", duration: "3:33" },
  { id: 177, title: "Flow Espacial", artist: "Matuê", duration: "2:59" },
  { id: 178, title: "A Morte do Autotune", artist: "Matuê", duration: "2:51" },
  { id: 179, title: "Bank", artist: "Matuê", duration: "2:42" },
  { id: 180, title: "Vampiro", artist: "Matuê, Teto, WIU", duration: "2:56" },
];

const lionelRichieSongs = [
  { id: 181, title: "Hello", artist: "Lionel Richie", duration: "4:08" },
  { id: 182, title: "All Night Long", artist: "Lionel Richie", duration: "6:25" },
  { id: 183, title: "Say You, Say Me", artist: "Lionel Richie", duration: "4:00" },
  { id: 184, title: "Stuck on You", artist: "Lionel Richie", duration: "3:10" },
  { id: 185, title: "Easy", artist: "Lionel Richie", duration: "4:21" },
  { id: 186, title: "Endless Love", artist: "Lionel Richie & Diana Ross", duration: "4:27" },
  { id: 187, title: "Dancing on the Ceiling", artist: "Lionel Richie", duration: "4:21" },
  { id: 188, title: "Truly", artist: "Lionel Richie", duration: "3:22" },
  { id: 189, title: "Penny Lover", artist: "Lionel Richie", duration: "3:46" },
  { id: 190, title: "You Are", artist: "Lionel Richie", duration: "4:03" },
];

const lanaDelReySongs = [
  { id: 191, title: "Summertime Sadness", artist: "Lana Del Rey", duration: "4:25" },
  { id: 192, title: "Young and Beautiful", artist: "Lana Del Rey", duration: "3:56" },
  { id: 193, title: "Born to Die", artist: "Lana Del Rey", duration: "4:46" },
  { id: 194, title: "Video Games", artist: "Lana Del Rey", duration: "4:42" },
  { id: 195, title: "West Coast", artist: "Lana Del Rey", duration: "4:16" },
  { id: 196, title: "Doin' Time", artist: "Lana Del Rey", duration: "3:21" },
  { id: 197, title: "Ride", artist: "Lana Del Rey", duration: "4:49" },
  { id: 198, title: "Blue Jeans", artist: "Lana Del Rey", duration: "3:30" },
  { id: 199, title: "Love", artist: "Lana Del Rey", duration: "4:33" },
  { id: 200, title: "Lust for Life", artist: "Lana Del Rey ft. The Weeknd", duration: "4:24" },
];

const katyPerrySongs = [
  { id: 201, title: "Firework", artist: "Katy Perry", duration: "3:48" },
  { id: 202, title: "Dark Horse", artist: "Katy Perry ft. Juicy J", duration: "3:35" },
  { id: 203, title: "Roar", artist: "Katy Perry", duration: "3:43" },
  { id: 204, title: "Teenage Dream", artist: "Katy Perry", duration: "3:48" },
  { id: 205, title: "Last Friday Night", artist: "Katy Perry", duration: "3:50" },
  { id: 206, title: "California Gurls", artist: "Katy Perry ft. Snoop Dogg", duration: "3:56" },
  { id: 207, title: "E.T.", artist: "Katy Perry ft. Kanye West", duration: "3:26" },
  { id: 208, title: "Hot N Cold", artist: "Katy Perry", duration: "3:40" },
  { id: 209, title: "Part of Me", artist: "Katy Perry", duration: "3:35" },
  { id: 210, title: "Wide Awake", artist: "Katy Perry", duration: "3:41" },
];

const justinBieberMoreSongs = [
  { id: 211, title: "Ghost", artist: "Justin Bieber", duration: "2:33" },
  { id: 212, title: "Hold On", artist: "Justin Bieber", duration: "2:51" },
  { id: 213, title: "No Brainer", artist: "DJ Khaled ft. Justin Bieber", duration: "4:20" },
  { id: 214, title: "Beauty and a Beat", artist: "Justin Bieber ft. Nicki Minaj", duration: "3:47" },
  { id: 215, title: "As Long As You Love Me", artist: "Justin Bieber ft. Big Sean", duration: "3:49" },
  { id: 216, title: "Boyfriend", artist: "Justin Bieber", duration: "2:52" },
  { id: 217, title: "Company", artist: "Justin Bieber", duration: "3:28" },
  { id: 218, title: "Confident", artist: "Justin Bieber ft. Chance the Rapper", duration: "4:00" },
  { id: 219, title: "Lonely", artist: "Justin Bieber & benny blanco", duration: "2:29" },
  { id: 220, title: "Honest", artist: "Justin Bieber ft. Don Toliver", duration: "3:13" },
];

const jorgeMateusSongs = [
  { id: 221, title: "Propaganda", artist: "Jorge & Mateus", duration: "3:30" },
  { id: 222, title: "Tijolão", artist: "Jorge & Mateus", duration: "3:13" },
  { id: 223, title: "Sosseguei", artist: "Jorge & Mateus", duration: "3:21" },
  { id: 224, title: "Vou Voando", artist: "Jorge & Mateus", duration: "3:35" },
  { id: 225, title: "Logo Eu", artist: "Jorge & Mateus", duration: "3:34" },
  { id: 226, title: "Medida Certa", artist: "Jorge & Mateus", duration: "3:45" },
  { id: 227, title: "Amo Noite e Dia", artist: "Jorge & Mateus", duration: "3:48" },
  { id: 228, title: "Pra Sempre com Você", artist: "Jorge & Mateus", duration: "3:52" },
  { id: 229, title: "Voa Beija-Flor", artist: "Jorge & Mateus", duration: "3:38" },
  { id: 230, title: "Pode Chorar", artist: "Jorge & Mateus", duration: "3:29" },
];

const jBalvinSongs = [
  { id: 231, title: "Mi Gente", artist: "J Balvin & Willy William", duration: "3:06" },
  { id: 232, title: "Ginza", artist: "J Balvin", duration: "2:49" },
  { id: 233, title: "Safari", artist: "J Balvin ft. Pharrell", duration: "3:27" },
  { id: 234, title: "X", artist: "J Balvin & Nicky Jam", duration: "2:53" },
  { id: 235, title: "Ay Vamos", artist: "J Balvin", duration: "3:28" },
  { id: 236, title: "Reggaeton", artist: "J Balvin", duration: "2:47" },
  { id: 237, title: "Ambiente", artist: "J Balvin", duration: "3:33" },
  { id: 238, title: "6 AM", artist: "J Balvin ft. Farruko", duration: "3:45" },
  { id: 239, title: "Tranquila", artist: "J Balvin", duration: "3:17" },
  { id: 240, title: "Loco Contigo", artist: "DJ Snake ft. J Balvin & Tyga", duration: "3:06" },
];

const izaSongs = [
  { id: 241, title: "Dona de Mim", artist: "IZA", duration: "3:36" },
  { id: 242, title: "Meu Talismã", artist: "IZA", duration: "3:38" },
  { id: 243, title: "Pesadão", artist: "IZA ft. Marcelo Falcão", duration: "3:07" },
  { id: 244, title: "Brisa", artist: "IZA", duration: "3:10" },
  { id: 245, title: "Ginga", artist: "IZA ft. Rincon Sapiência", duration: "3:26" },
  { id: 246, title: "Fé nas Malucas", artist: "IZA ft. MC Carol", duration: "2:59" },
  { id: 247, title: "Sem Filtro", artist: "IZA", duration: "3:15" },
  { id: 248, title: "Gueto", artist: "IZA", duration: "3:03" },
  { id: 249, title: "Droga", artist: "IZA", duration: "3:42" },
  { id: 250, title: "Esse Brilho é Meu", artist: "IZA", duration: "2:51" },
];

const imagineDragonsSongs = [
  { id: 251, title: "Believer", artist: "Imagine Dragons", duration: "3:24" },
  { id: 252, title: "Demons", artist: "Imagine Dragons", duration: "2:57" },
  { id: 253, title: "Radioactive", artist: "Imagine Dragons", duration: "3:06" },
  { id: 254, title: "Thunder", artist: "Imagine Dragons", duration: "3:07" },
  { id: 255, title: "Whatever It Takes", artist: "Imagine Dragons", duration: "3:21" },
  { id: 256, title: "Enemy", artist: "Imagine Dragons ft. JID", duration: "2:53" },
  { id: 257, title: "Natural", artist: "Imagine Dragons", duration: "3:10" },
  { id: 258, title: "On Top of the World", artist: "Imagine Dragons", duration: "3:12" },
  { id: 259, title: "It's Time", artist: "Imagine Dragons", duration: "4:00" },
  { id: 260, title: "Bad Liar", artist: "Imagine Dragons", duration: "4:20" },
];

const harryStylesSongs = [
  { id: 261, title: "As It Was", artist: "Harry Styles", duration: "2:47" },
  { id: 262, title: "Watermelon Sugar", artist: "Harry Styles", duration: "2:54" },
  { id: 263, title: "Sign of the Times", artist: "Harry Styles", duration: "5:41" },
  { id: 264, title: "Falling", artist: "Harry Styles", duration: "4:00" },
  { id: 265, title: "Late Night Talking", artist: "Harry Styles", duration: "2:57" },
  { id: 266, title: "Adore You", artist: "Harry Styles", duration: "3:27" },
  { id: 267, title: "Golden", artist: "Harry Styles", duration: "3:28" },
  { id: 268, title: "Music for a Sushi Restaurant", artist: "Harry Styles", duration: "3:14" },
  { id: 269, title: "Daylight", artist: "Harry Styles", duration: "2:44" },
  { id: 270, title: "Matilda", artist: "Harry Styles", duration: "4:05" },
];

const nattanzinhoSongs = [
  { id: 271, title: "Love Gostosinho", artist: "Nattan & Mari Fernandez", duration: "2:44" },
  { id: 272, title: "Tem Cabaré Essa Noite", artist: "Nattan & Nivaldo Marques", duration: "2:50" },
  { id: 273, title: "Storiezin", artist: "Nattan", duration: "2:58" },
  { id: 274, title: "Comunicação Falhou", artist: "Nattan", duration: "3:10" },
  { id: 275, title: "Não te Quero", artist: "Nattan", duration: "2:45" },
  { id: 276, title: "Manda Áudio", artist: "Nattan", duration: "2:49" },
  { id: 277, title: "Se For Amor", artist: "Nattan", duration: "2:54" },
  { id: 278, title: "Te Amar foi Ilusão", artist: "Nattan", duration: "3:00" },
  { id: 279, title: "Diferente das Diferentes", artist: "Nattan", duration: "3:03" },
  { id: 280, title: "Diz Aí", artist: "Nattan", duration: "2:40" },
];

const gusttavoLimaSongs = [
  { id: 281, title: "Apelido Carinhoso", artist: "Gusttavo Lima", duration: "3:28" },
  { id: 282, title: "Zé da Recaída", artist: "Gusttavo Lima", duration: "3:21" },
  { id: 283, title: "Fala Mal de Mim", artist: "Gusttavo Lima", duration: "3:18" },
  { id: 284, title: "Bloqueado", artist: "Gusttavo Lima", duration: "2:56" },
  { id: 285, title: "Milu", artist: "Gusttavo Lima", duration: "3:12" },
  { id: 286, title: "A Gente Fez Amor", artist: "Gusttavo Lima", duration: "3:10" },
  { id: 287, title: "Balada (Tchê Tcherere Tchê Tchê)", artist: "Gusttavo Lima", duration: "3:23" },
  { id: 288, title: "Termina Comigo Antes", artist: "Gusttavo Lima", duration: "3:07" },
  { id: 289, title: "Desejo Imortal", artist: "Gusttavo Lima", duration: "3:33" },
  { id: 290, title: "Homem de Família", artist: "Gusttavo Lima", duration: "3:15" },
];

const brunoMarsSongs = [
  { id: 291, title: "Uptown Funk", artist: "Mark Ronson ft. Bruno Mars", duration: "4:30" },
  { id: 292, title: "That's What I Like", artist: "Bruno Mars", duration: "3:26" },
  { id: 293, title: "Just the Way You Are", artist: "Bruno Mars", duration: "3:41" },
  { id: 294, title: "Locked Out of Heaven", artist: "Bruno Mars", duration: "3:53" },
  { id: 295, title: "Treasure", artist: "Bruno Mars", duration: "2:58" },
  { id: 296, title: "24K Magic", artist: "Bruno Mars", duration: "3:45" },
  { id: 297, title: "Grenade", artist: "Bruno Mars", duration: "3:42" },
  { id: 298, title: "Finesse", artist: "Bruno Mars ft. Cardi B", duration: "3:11" },
  { id: 299, title: "When I Was Your Man", artist: "Bruno Mars", duration: "3:33" },
  { id: 300, title: "Leave The Door Open", artist: "Silk Sonic (Bruno Mars & Anderson .Paak)", duration: "4:02" },
];

const fiftyCentSongs = [
  { id: 301, title: "In Da Club", artist: "50 Cent", duration: "3:13" },
  { id: 302, title: "Candy Shop", artist: "50 Cent ft. Olivia", duration: "3:29" },
  { id: 303, title: "21 Questions", artist: "50 Cent ft. Nate Dogg", duration: "3:44" },
  { id: 304, title: "Just a Lil Bit", artist: "50 Cent", duration: "3:57" },
  { id: 305, title: "Many Men (Wish Death)", artist: "50 Cent", duration: "4:16" },
  { id: 306, title: "Window Shopper", artist: "50 Cent", duration: "3:11" },
  { id: 307, title: "P.I.M.P.", artist: "50 Cent", duration: "4:09" },
  { id: 308, title: "Ayo Technology", artist: "50 Cent ft. Justin Timberlake", duration: "4:10" },
  { id: 309, title: "Outta Control", artist: "50 Cent", duration: "3:23" },
  { id: 310, title: "Hustler's Ambition", artist: "50 Cent", duration: "3:59" },
];

const abbaSongs = [
  { id: 311, title: "Dancing Queen", artist: "ABBA", duration: "3:52" },
  { id: 312, title: "Mamma Mia", artist: "ABBA", duration: "3:33" },
  { id: 313, title: "Gimme! Gimme! Gimme!", artist: "ABBA", duration: "3:58" },
  { id: 314, title: "Take a Chance on Me", artist: "ABBA", duration: "4:05" },
  { id: 315, title: "Waterloo", artist: "ABBA", duration: "2:46" },
  { id: 316, title: "The Winner Takes It All", artist: "ABBA", duration: "4:56" },
  { id: 317, title: "Fernando", artist: "ABBA", duration: "4:14" },
  { id: 318, title: "S.O.S.", artist: "ABBA", duration: "3:22" },
  { id: 319, title: "Knowing Me, Knowing You", artist: "ABBA", duration: "4:02" },
  { id: 320, title: "Super Trouper", artist: "ABBA", duration: "4:13" },
];

const acdcSongs = [
  { id: 321, title: "Back In Black", artist: "AC/DC", duration: "4:15" },
  { id: 322, title: "Highway to Hell", artist: "AC/DC", duration: "3:28" },
  { id: 323, title: "Thunderstruck", artist: "AC/DC", duration: "4:52" },
  { id: 324, title: "You Shook Me All Night Long", artist: "AC/DC", duration: "3:30" },
  { id: 325, title: "T.N.T.", artist: "AC/DC", duration: "3:34" },
  { id: 326, title: "Shoot to Thrill", artist: "AC/DC", duration: "5:17" },
  { id: 327, title: "Hells Bells", artist: "AC/DC", duration: "5:12" },
  { id: 328, title: "Dirty Deeds Done Dirt Cheap", artist: "AC/DC", duration: "3:52" },
  { id: 329, title: "For Those About to Rock", artist: "AC/DC", duration: "5:44" },
  { id: 330, title: "Rock and Roll Ain’t Noise Pollution", artist: "AC/DC", duration: "4:15" },
];

const akonSongs = [
  { id: 331, title: "Smack That", artist: "Akon ft. Eminem", duration: "3:32" },
  { id: 332, title: "Lonely", artist: "Akon", duration: "3:55" },
  { id: 333, title: "Right Now (Na Na Na)", artist: "Akon", duration: "4:01" },
  { id: 334, title: "Don't Matter", artist: "Akon", duration: "4:52" },
  { id: 335, title: "Beautiful", artist: "Akon ft. Colby O'Donis", duration: "5:14" },
  { id: 336, title: "I Wanna Love You", artist: "Akon ft. Snoop Dogg", duration: "4:07" },
  { id: 337, title: "Sorry, Blame It On Me", artist: "Akon", duration: "4:57" },
  { id: 338, title: "Ghetto", artist: "Akon", duration: "4:25" },
  { id: 339, title: "Locked Up", artist: "Akon ft. Styles P", duration: "3:57" },
  { id: 340, title: "Bananza (Belly Dancer)", artist: "Akon", duration: "4:00" },
];

const anittaSongs = [
  { id: 341, title: "Envolver", artist: "Anitta", duration: "3:13" },
  { id: 342, title: "Show das Poderosas", artist: "Anitta", duration: "2:56" },
  { id: 343, title: "Bang", artist: "Anitta", duration: "3:42" },
  { id: 344, title: "Vai Malandra", artist: "Anitta ft. MC Zaac", duration: "3:01" },
  { id: 345, title: "Paradinha", artist: "Anitta", duration: "3:10" },
  { id: 346, title: "Downtown", artist: "Anitta & J Balvin", duration: "3:05" },
  { id: 347, title: "Girl from Rio", artist: "Anitta", duration: "3:13" },
  { id: 348, title: "Me Gusta", artist: "Anitta ft. Cardi B & Myke Towers", duration: "3:08" },
  { id: 349, title: "Funk Rave", artist: "Anitta", duration: "2:30" },
  { id: 350, title: "Essa Mina é Louca", artist: "Anitta ft. Jhama", duration: "2:55" },
];

const arianaGrandeSongs = [
  { id: 351, title: "7 rings", artist: "Ariana Grande", duration: "2:59" },
  { id: 352, title: "Thank U, Next", artist: "Ariana Grande", duration: "3:27" },
  { id: 353, title: "No Tears Left to Cry", artist: "Ariana Grande", duration: "3:26" },
  { id: 354, title: "Into You", artist: "Ariana Grande", duration: "4:04" },
  { id: 355, title: "Positions", artist: "Ariana Grande", duration: "2:52" },
  { id: 356, title: "Dangerous Woman", artist: "Ariana Grande", duration: "3:55" },
  { id: 357, title: "Side to Side", artist: "Ariana Grande ft. Nicki Minaj", duration: "3:46" },
  { id: 358, title: "Break Free", artist: "Ariana Grande ft. Zedd", duration: "3:34" },
  { id: 359, title: "Problem", artist: "Ariana Grande ft. Iggy Azalea", duration: "3:14" },
  { id: 360, title: "God is a woman", artist: "Ariana Grande", duration: "3:17" },
];

const badBunnySongs = [
  { id: 361, title: "Tití Me Preguntó", artist: "Bad Bunny", duration: "4:03" },
  { id: 362, title: "Me Porto Bonito", artist: "Bad Bunny & Chencho Corleone", duration: "2:58" },
  { id: 363, title: "Ojitos Lindos", artist: "Bad Bunny & Bomba Estéreo", duration: "4:18" },
  { id: 364, title: "Moscow Mule", artist: "Bad Bunny", duration: "4:05" },
  { id: 365, title: "Callaita", artist: "Bad Bunny & Tainy", duration: "4:10" },
  { id: 366, title: "Yonaguni", artist: "Bad Bunny", duration: "3:26" },
  { id: 367, title: "DÁKITI", artist: "Bad Bunny & Jhay Cortez", duration: "3:25" },
  { id: 368, title: "La Canción", artist: "Bad Bunny & J Balvin", duration: "4:02" },
  { id: 369, title: "Amorfoda", artist: "Bad Bunny", duration: "3:31" },
  { id: 370, title: "Un Preview", artist: "Bad Bunny", duration: "2:46" },
];

const beyonceSongs = [
  { id: 371, title: "Halo", artist: "Beyoncé", duration: "3:44" },
  { id: 372, title: "Crazy in Love", artist: "Beyoncé ft. Jay-Z", duration: "3:56" },
  { id: 373, title: "Single Ladies", artist: "Beyoncé", duration: "3:13" },
  { id: 374, title: "Cuff It", artist: "Beyoncé", duration: "3:45" },
  { id: 375, title: "Break My Soul", artist: "Beyoncé", duration: "4:38" },
  { id: 376, title: "Irreplaceable", artist: "Beyoncé", duration: "3:47" },
  { id: 377, title: "Love On Top", artist: "Beyoncé", duration: "4:27" },
  { id: 378, title: "Run the World (Girls)", artist: "Beyoncé", duration: "3:55" },
  { id: 379, title: "Drunk in Love", artist: "Beyoncé ft. Jay-Z", duration: "5:23" },
  { id: 380, title: "Formation", artist: "Beyoncé", duration: "3:26" },
];

const billieEilishSongs = [
  { id: 381, title: "Bad Guy", artist: "Billie Eilish", duration: "3:14" },
  { id: 382, title: "Happier Than Ever", artist: "Billie Eilish", duration: "4:58" },
  { id: 383, title: "Everything I Wanted", artist: "Billie Eilish", duration: "4:05" },
  { id: 384, title: "Lovely", artist: "Billie Eilish & Khalid", duration: "3:20" },
  { id: 385, title: "Ocean Eyes", artist: "Billie Eilish", duration: "3:20" },
  { id: 386, title: "When the Party's Over", artist: "Billie Eilish", duration: "3:16" },
  { id: 387, title: "Bury a Friend", artist: "Billie Eilish", duration: "3:13" },
  { id: 388, title: "You Should See Me in a Crown", artist: "Billie Eilish", duration: "3:00" },
  { id: 389, title: "My Future", artist: "Billie Eilish", duration: "3:28" },
  { id: 390, title: "I Love You", artist: "Billie Eilish", duration: "4:52" },
];

const britneySpearsSongs = [
  { id: 391, title: "...Baby One More Time", artist: "Britney Spears", duration: "3:30" },
  { id: 392, title: "Toxic", artist: "Britney Spears", duration: "3:20" },
  { id: 393, title: "Oops!... I Did It Again", artist: "Britney Spears", duration: "3:32" },
  { id: 394, title: "Gimme More", artist: "Britney Spears", duration: "4:11" },
  { id: 395, title: "Womanizer", artist: "Britney Spears", duration: "3:44" },
  { id: 396, title: "Circus", artist: "Britney Spears", duration: "3:12" },
  { id: 397, title: "Stronger", artist: "Britney Spears", duration: "3:23" },
  { id: 398, title: "I'm a Slave 4 U", artist: "Britney Spears", duration: "3:23" },
  { id: 399, title: "Everytime", artist: "Britney Spears", duration: "3:53" },
  { id: 400, title: "Criminal", artist: "Britney Spears", duration: "3:45" },
];

const cardiBSongs = [
  { id: 401, title: "WAP", artist: "Cardi B ft. Megan Thee Stallion", duration: "3:07" },
  { id: 402, title: "I Like It", artist: "Cardi B ft. Bad Bunny & J Balvin", duration: "4:13" },
  { id: 403, title: "Bodak Yellow", artist: "Cardi B", duration: "3:44" },
  { id: 404, title: "Up", artist: "Cardi B", duration: "2:36" },
  { id: 405, title: "Be Careful", artist: "Cardi B", duration: "3:30" },
  { id: 406, title: "Bartier Cardi", artist: "Cardi B ft. 21 Savage", duration: "3:44" },
  { id: 407, title: "Press", artist: "Cardi B", duration: "2:24" },
  { id: 408, title: "Money", artist: "Cardi B", duration: "3:03" },
  { id: 409, title: "Please Me", artist: "Cardi B & Bruno Mars", duration: "3:21" },
  { id: 410, title: "Hot Sh*t", artist: "Cardi B ft. Kanye West & Lil Durk", duration: "3:42" },
];

const coldplaySongs = [
  { id: 411, title: "Yellow", artist: "Coldplay", duration: "4:29" },
  { id: 412, title: "Viva La Vida", artist: "Coldplay", duration: "4:02" },
  { id: 413, title: "Fix You", artist: "Coldplay", duration: "4:55" },
  { id: 414, title: "The Scientist", artist: "Coldplay", duration: "5:09" },
  { id: 415, title: "Paradise", artist: "Coldplay", duration: "4:39" },
  { id: 416, title: "A Sky Full of Stars", artist: "Coldplay", duration: "4:28" },
  { id: 417, title: "Something Just Like This", artist: "Coldplay & The Chainsmokers", duration: "4:08" },
  { id: 418, title: "Clocks", artist: "Coldplay", duration: "5:07" },
  { id: 419, title: "Adventure of a Lifetime", artist: "Coldplay", duration: "4:23" },
  { id: 420, title: "My Universe", artist: "Coldplay & BTS", duration: "3:48" },
];

const chrisBrownSongs = [
  { id: 421, title: "Under the Influence", artist: "Chris Brown", duration: "3:04" },
  { id: 422, title: "No Guidance", artist: "Chris Brown ft. Drake", duration: "4:21" },
  { id: 423, title: "Forever", artist: "Chris Brown", duration: "4:38" },
  { id: 424, title: "With You", artist: "Chris Brown", duration: "4:13" },
  { id: 425, title: "Loyal", artist: "Chris Brown ft. Lil Wayne & Tyga", duration: "4:24" },
  { id: 426, title: "Say Goodbye", artist: "Chris Brown", duration: "4:50" },
  { id: 427, title: "Look at Me Now", artist: "Chris Brown ft. Busta Rhymes & Lil Wayne", duration: "3:43" },
  { id: 428, title: "Kiss Kiss", artist: "Chris Brown ft. T-Pain", duration: "4:10" },
  { id: 429, title: "Don't Wake Me Up", artist: "Chris Brown", duration: "3:42" },
  { id: 430, title: "Turn Up the Music", artist: "Chris Brown", duration: "3:47" },
];

const djavanSongs = [
  { id: 431, title: "Oceano", artist: "Djavan", duration: "3:50" },
  { id: 432, title: "Sina", artist: "Djavan", duration: "4:03" },
  { id: 433, title: "Flor de Lis", artist: "Djavan", duration: "3:44" },
  { id: 434, title: "Meu Bem Querer", artist: "Djavan", duration: "4:11" },
  { id: 435, title: "Se...", artist: "Djavan", duration: "4:07" },
  { id: 436, title: "Samurai", artist: "Djavan", duration: "4:35" },
  { id: 437, title: "Lilás", artist: "Djavan", duration: "3:45" },
  { id: 438, title: "Açaí", artist: "Djavan", duration: "3:39" },
  { id: 439, title: "Nem Um Dia", artist: "Djavan", duration: "3:53" },
  { id: 440, title: "Pétala", artist: "Djavan", duration: "4:28" },
];

const dojaCatSongs = [
  { id: 441, title: "Say So", artist: "Doja Cat", duration: "3:57" },
  { id: 442, title: "Woman", artist: "Doja Cat", duration: "2:52" },
  { id: 443, title: "Paint the Town Red", artist: "Doja Cat", duration: "3:51" },
  { id: 444, title: "Need to Know", artist: "Doja Cat", duration: "3:31" },
  { id: 445, title: "Kiss Me More", artist: "Doja Cat ft. SZA", duration: "3:28" },
  { id: 446, title: "Like That", artist: "Doja Cat ft. Gucci Mane", duration: "2:42" },
  { id: 447, title: "Vegas", artist: "Doja Cat", duration: "3:02" },
  { id: 448, title: "Streets", artist: "Doja Cat", duration: "3:46" },
  { id: 449, title: "Ain’t Sh*t", artist: "Doja Cat", duration: "2:54" },
  { id: 450, title: "Get Into It (Yuh)", artist: "Doja Cat", duration: "2:18" },
];

const drDreSongs = [
  { id: 451, title: "Forgot About Dre", artist: "Dr. Dre ft. Eminem", duration: "3:42" },
  { id: 452, title: "Still D.R.E.", artist: "Dr. Dre ft. Snoop Dogg", duration: "4:31" },
  { id: 453, title: "The Next Episode", artist: "Dr. Dre ft. Snoop Dogg", duration: "2:41" },
  { id: 454, title: "Nuthin' But a G Thang", artist: "Dr. Dre ft. Snoop Dogg", duration: "3:58" },
  { id: 455, title: "Let Me Ride", artist: "Dr. Dre", duration: "4:21" },
  { id: 456, title: "Kush", artist: "Dr. Dre ft. Snoop Dogg & Akon", duration: "3:44" },
  { id: 457, title: "Keep Their Heads Ringin'", artist: "Dr. Dre", duration: "5:07" },
  { id: 458, title: "I Need a Doctor", artist: "Dr. Dre ft. Eminem & Skylar Grey", duration: "4:44" },
  { id: 459, title: "Xxplosive", artist: "Dr. Dre", duration: "3:46" },
  { id: 460, title: "Deep Cover", artist: "Dr. Dre & Snoop Dogg", duration: "4:07" },
];

const drakeSongs = [
  { id: 461, title: "One Dance", artist: "Drake ft. Wizkid & Kyla", duration: "2:54" },
  { id: 462, title: "God's Plan", artist: "Drake", duration: "3:18" },
  { id: 463, title: "Hotline Bling", artist: "Drake", duration: "4:27" },
  { id: 464, title: "In My Feelings", artist: "Drake", duration: "3:37" },
  { id: 465, title: "Passionfruit", artist: "Drake", duration: "4:58" },
  { id: 466, title: "Nonstop", artist: "Drake", duration: "3:59" },
  { id: 467, title: "Nice for What", artist: "Drake", duration: "3:30" },
  { id: 468, title: "Started From the Bottom", artist: "Drake", duration: "2:53" },
  { id: 469, title: "Jimmy Cooks", artist: "Drake ft. 21 Savage", duration: "3:38" },
  { id: 470, title: "Laugh Now Cry Later", artist: "Drake ft. Lil Durk", duration: "4:21" },
];

const duaLipaSongs = [
  { id: 471, title: "Levitating", artist: "Dua Lipa", duration: "3:23" },
  { id: 472, title: "Don't Start Now", artist: "Dua Lipa", duration: "3:03" },
  { id: 473, title: "New Rules", artist: "Dua Lipa", duration: "3:29" },
  { id: 474, title: "Physical", artist: "Dua Lipa", duration: "3:14" },
  { id: 475, title: "One Kiss", artist: "Calvin Harris & Dua Lipa", duration: "3:35" },
  { id: 476, title: "Break My Heart", artist: "Dua Lipa", duration: "3:41" },
  { id: 477, title: "Love Again", artist: "Dua Lipa", duration: "4:18" },
  { id: 478, title: "IDGAF", artist: "Dua Lipa", duration: "3:37" },
  { id: 479, title: "Be the One", artist: "Dua Lipa", duration: "3:24" },
  { id: 480, title: "Dance The Night", artist: "Dua Lipa", duration: "2:56" },
];

const edSheeranSongs = [
  { id: 481, title: "Shape of You", artist: "Ed Sheeran", duration: "3:53" },
  { id: 482, title: "Perfect", artist: "Ed Sheeran", duration: "4:23" },
  { id: 483, title: "Thinking Out Loud", artist: "Ed Sheeran", duration: "4:41" },
  { id: 484, title: "Photograph", artist: "Ed Sheeran", duration: "4:18" },
  { id: 485, title: "Bad Habits", artist: "Ed Sheeran", duration: "3:51" },
  { id: 486, title: "Shivers", artist: "Ed Sheeran", duration: "3:27" },
  { id: 487, title: "Eyes Closed", artist: "Ed Sheeran", duration: "3:15" },
  { id: 488, title: "Castle on the Hill", artist: "Ed Sheeran", duration: "4:21" },
  { id: 489, title: "Afterglow", artist: "Ed Sheeran", duration: "3:05" },
  { id: 490, title: "Galway Girl", artist: "Ed Sheeran", duration: "2:50" },
];

const eltonJohnSongs = [
  { id: 491, title: "Rocket Man", artist: "Elton John", duration: "4:41" },
  { id: 492, title: "Tiny Dancer", artist: "Elton John", duration: "6:17" },
  { id: 493, title: "Your Song", artist: "Elton John", duration: "4:00" },
  { id: 494, title: "I'm Still Standing", artist: "Elton John", duration: "3:02" },
  { id: 495, title: "Sacrifice", artist: "Elton John", duration: "5:06" },
  { id: 496, title: "Don't Go Breaking My Heart", artist: "Elton John & Kiki Dee", duration: "4:25" },
  { id: 497, title: "Cold Heart (PNAU Remix)", artist: "Elton John & Dua Lipa", duration: "3:22" },
  { id: 498, title: "Bennie and the Jets", artist: "Elton John", duration: "5:23" },
  { id: 499, title: "Candle in the Wind", artist: "Elton John", duration: "3:50" },
  { id: 500, title: "Goodbye Yellow Brick Road", artist: "Elton John", duration: "3:13" },
];

const eminemSongs = [
  { id: 501, title: "Lose Yourself", artist: "Eminem", duration: "5:20" },
  { id: 502, title: "The Real Slim Shady", artist: "Eminem", duration: "4:44" },
  { id: 503, title: "Without Me", artist: "Eminem", duration: "4:50" },
  { id: 504, title: "Mockingbird", artist: "Eminem", duration: "4:11" },
  { id: 505, title: "Stan", artist: "Eminem ft. Dido", duration: "6:44" },
  { id: 506, title: "Love The Way You Lie", artist: "Eminem ft. Rihanna", duration: "4:23" },
  { id: 507, title: "Not Afraid", artist: "Eminem", duration: "4:10" },
  { id: 508, title: "Godzilla", artist: "Eminem ft. Juice WRLD", duration: "3:31" },
  { id: 509, title: "Rap God", artist: "Eminem", duration: "6:03" },
  { id: 510, title: "When I'm Gone", artist: "Eminem", duration: "4:40" },
];

const henriqueJulianoSongs = [
  { id: 511, title: "Cuida Bem Dela", artist: "Henrique & Juliano", duration: "3:26" },
  { id: 512, title: "Liberdade Provisória", artist: "Henrique & Juliano", duration: "3:16" },
  { id: 513, title: "Vidinha de Balada", artist: "Henrique & Juliano", duration: "3:05" },
  { id: 514, title: "Até Você Voltar", artist: "Henrique & Juliano", duration: "3:38" },
  { id: 515, title: "Recaídas", artist: "Henrique & Juliano", duration: "3:20" },
  { id: 516, title: "Na Hora da Raiva", artist: "Henrique & Juliano", duration: "3:45" },
  { id: 517, title: "Flor e o Beija-Flor", artist: "Henrique & Juliano", duration: "3:49" },
  { id: 518, title: "A Hora da Raiva", artist: "Henrique & Juliano", duration: "3:15" },
  { id: 519, title: "Vidinha de Balada (Acústico)", artist: "Henrique & Juliano", duration: "3:02" },
  { id: 520, title: "Flor e o Beija-Flor (Acústico)", artist: "Henrique & Juliano", duration: "3:51" },
];

const luanSantanaSongs = [
  { id: 521, title: "Meteoro", artist: "Luan Santana", duration: "3:42" },
  { id: 522, title: "Te Esperando", artist: "Luan Santana", duration: "3:36" },
  { id: 523, title: "Sogrão Caprichou", artist: "Luan Santana", duration: "3:40" },
  { id: 524, title: "Quando Chega a Noite", artist: "Luan Santana", duration: "3:10" },
  { id: 525, title: "Escreve Aí", artist: "Luan Santana", duration: "3:54" },
  { id: 526, title: "Vai Fazer Falta", artist: "Luan Santana", duration: "3:22" },
  { id: 527, title: "Cê Topa", artist: "Luan Santana", duration: "3:05" },
  { id: 528, title: "Amar Não É Pecado", artist: "Luan Santana", duration: "3:31" },
  { id: 529, title: "Sinais", artist: "Luan Santana", duration: "3:40" },
  { id: 530, title: "Tudo Que Você Quiser", artist: "Luan Santana", duration: "3:56" },
];

const matheusKauanSongs = [
  { id: 531, title: "Quem Pegou, Pegou", artist: "Matheus & Kauan", duration: "3:21" },
  { id: 532, title: "O Nosso Santo Bateu", artist: "Matheus & Kauan", duration: "3:46" },
  { id: 533, title: "Tudo Que Tem Que Ser", artist: "Matheus & Kauan", duration: "3:36" },
  { id: 534, title: "Que Sorte a Nossa", artist: "Matheus & Kauan", duration: "3:28" },
  { id: 535, title: "Vou Ter Que Superar", artist: "Matheus & Kauan", duration: "3:33" },
  { id: 536, title: "Na Conta da Loucura", artist: "Matheus & Kauan", duration: "3:54" },
  { id: 537, title: "Vamos Falar de Amor", artist: "Matheus & Kauan", duration: "3:25" },
  { id: 538, title: "Te Assumi Pro Brasil", artist: "Matheus & Kauan", duration: "3:45" },
  { id: 539, title: "Cerveja, Sal e Limão", artist: "Matheus & Kauan", duration: "3:40" },
  { id: 540, title: "Eu Já Tava Bem", artist: "Matheus & Kauan", duration: "3:50" },
];

const mcTutoSongs = [
  { id: 541, title: "Chama o Síndico", artist: "MC Tuto", duration: "3:20" },
  { id: 542, title: "Te Quero do Jeito Que Quiser", artist: "MC Tuto", duration: "3:25" },
  { id: 543, title: "Vai Levando", artist: "MC Tuto", duration: "3:15" },
  { id: 544, title: "Descontrolada", artist: "MC Tuto", duration: "3:35" },
  { id: 545, title: "Deu Ruim", artist: "MC Tuto", duration: "3:05" },
  { id: 546, title: "Bem Melhor", artist: "MC Tuto", duration: "3:18" },
  { id: 547, title: "Fazendo o Movimento", artist: "MC Tuto", duration: "3:22" },
  { id: 548, title: "Tchau", artist: "MC Tuto", duration: "3:12" },
  { id: 549, title: "Recaída", artist: "MC Tuto", duration: "3:16" },
  { id: 550, title: "Dona da Noite", artist: "MC Tuto", duration: "3:45" },
];

const maroon5Songs = [
  { id: 551, title: "Sugar", artist: "Maroon 5", duration: "3:55" },
  { id: 552, title: "Girls Like You", artist: "Maroon 5 ft. Cardi B", duration: "3:55" },
  { id: 553, title: "Memories", artist: "Maroon 5", duration: "3:09" },
  { id: 554, title: "Moves Like Jagger", artist: "Maroon 5 ft. Christina Aguilera", duration: "3:21" },
  { id: 555, title: "One More Night", artist: "Maroon 5", duration: "3:40" },
  { id: 556, title: "Animals", artist: "Maroon 5", duration: "3:50" },
  { id: 557, title: "This Love", artist: "Maroon 5", duration: "3:26" },
  { id: 558, title: "Payphone", artist: "Maroon 5 ft. Wiz Khalifa", duration: "3:51" },
  { id: 559, title: "Maps", artist: "Maroon 5", duration: "3:39" },
  { id: 560, title: "Cold", artist: "Maroon 5 ft. Future", duration: "3:32" },
];

const rihannaSongs = [
  { id: 561, title: "Umbrella", artist: "Rihanna ft. Jay-Z", duration: "4:35" },
  { id: 562, title: "Diamonds", artist: "Rihanna", duration: "3:45" },
  { id: 563, title: "Work", artist: "Rihanna ft. Drake", duration: "3:39" },
  { id: 564, title: "Only Girl (In the World)", artist: "Rihanna", duration: "3:55" },
  { id: 565, title: "Rude Boy", artist: "Rihanna", duration: "3:43" },
  { id: 566, title: "Stay", artist: "Rihanna ft. Mikky Ekko", duration: "4:00" },
  { id: 567, title: "We Found Love", artist: "Rihanna ft. Calvin Harris", duration: "3:35" },
  { id: 568, title: "Don't Stop the Music", artist: "Rihanna", duration: "4:27" },
  { id: 569, title: "SOS", artist: "Rihanna", duration: "4:00" },
  { id: 570, title: "Love on the Brain", artist: "Rihanna", duration: "3:44" },
];

const luisFonsiSongs = [
  { id: 571, title: "Despacito", artist: "Luis Fonsi ft. Daddy Yankee", duration: "3:48" },
  { id: 572, title: "Échame La Culpa", artist: "Luis Fonsi & Demi Lovato", duration: "2:53" },
  { id: 573, title: "Imposible", artist: "Luis Fonsi ft. Ozuna", duration: "3:37" },
  { id: 574, title: "Nada Es Para Siempre", artist: "Luis Fonsi", duration: "3:57" },
  { id: 575, title: "Aquí Estoy Yo", artist: "Luis Fonsi", duration: "4:16" },
  { id: 576, title: "No Me Doy Por Vencido", artist: "Luis Fonsi", duration: "4:15" },
  { id: 577, title: "Corazón En La Maleta", artist: "Luis Fonsi", duration: "3:38" },
  { id: 578, title: "Se Supone", artist: "Luis Fonsi", duration: "3:39" },
  { id: 579, title: "Por Una Mujer", artist: "Luis Fonsi", duration: "4:00" },
  { id: 580, title: "Gritar", artist: "Luis Fonsi", duration: "3:55" },
];

const blackpinkSongs = [
  { id: 581, title: "Ddu-Du Ddu-Du", artist: "BLACKPINK", duration: "3:29" },
  { id: 582, title: "Kill This Love", artist: "BLACKPINK", duration: "3:37" },
  { id: 583, title: "How You Like That", artist: "BLACKPINK", duration: "3:01" },
  { id: 584, title: "Lovesick Girls", artist: "BLACKPINK", duration: "3:23" },
  { id: 585, title: "As If It's Your Last", artist: "BLACKPINK", duration: "3:31" },
  { id: 586, title: "Whistle", artist: "BLACKPINK", duration: "3:34" },
  { id: 587, title: "JUMP", artist: "BLACKPINK", duration: "2:44"},
  { id: 588, title: "Pink Venom", artist: "BLACKPINK", duration: "3:19"},
  { id: 589, title: "STAY", artist: "BLACKPINK", duration: "3:50"},
  { id: 590, title: "Forever Young", artist: "BLACKPINK", duration: "3:59"}
];

const menosEMaisSongs = [
  { id: 591, title: "Melhor Eu Ir", artist: "Menos é Mais", duration: "3:31" },
  { id: 592, title: "Adorei Te Ver / Ainda Gosto Dela", artist: "Menos é Mais", duration: "4:45" },
  { id: 593, title: "Ligando os Fatos", artist: "Menos é Mais", duration: "3:33" },
  { id: 594, title: "Péssimo Negócio", artist: "Menos é Mais", duration: "3:47" },
  { id: 595, title: "Vou Ver e Te Aviso", artist: "Menos é Mais", duration: "3:41" },
  { id: 596, title: "Recaída / Mineirinho / Vai Embora", artist: "Menos é Mais", duration: "4:01" },
  { id: 597, title: "Melhor Eu Ir / Vai Me Dando Corda", artist: "Menos é Mais", duration: "4:12" },
  { id: 598, title: "Pra Você Acreditar", artist: "Menos é Mais", duration: "3:58" },
  { id: 599, title: "Lapada Dela", artist: "Menos é Mais", duration: "2:44" },
  { id: 600, title: "Melhor Eu Ir / Ligando os Fatos", artist: "Menos é Mais", duration: "4:10" },
];

const joseRicoMilionarioSongs = [
  { id: 601, title: "Estrada da Vida", artist: "José Rico & Milionário", duration: "3:23" },
  { id: 602, title: "Sonho de Caboclo", artist: "José Rico & Milionário", duration: "3:40" },
  { id: 603, title: "Pense em Mim", artist: "José Rico & Milionário", duration: "3:35" },
  { id: 604, title: "Ainda Ontem Chorei de Saudade", artist: "José Rico & Milionário", duration: "3:47" },
  { id: 605, title: "Saudade de Minha Terra", artist: "José Rico & Milionário", duration: "3:55" },
  { id: 606, title: "Duas Lágrimas", artist: "José Rico & Milionário", duration: "3:25" },
  { id: 607, title: "Frio da Solidão", artist: "José Rico & Milionário", duration: "3:40" },
  { id: 608, title: "Cachaça", artist: "José Rico & Milionário", duration: "3:30" },
  { id: 609, title: "Fui Eu", artist: "José Rico & Milionário", duration: "3:36" },
  { id: 610, title: "Comitiva Esperança", artist: "José Rico & Milionário", duration: "3:44" },
];

const boxMedleySongs = [
  { id: 611, title: "Medley Sertanejo", artist: "Box Medley", duration: "10:00" },
  { id: 612, title: "Medley Arrocha", artist: "Box Medley", duration: "9:30" },
  { id: 613, title: "Medley Forró", artist: "Box Medley", duration: "8:45" },
  { id: 614, title: "Medley Brega", artist: "Box Medley", duration: "9:15" },
  { id: 615, title: "Medley Pagode", artist: "Box Medley", duration: "9:50" },
  { id: 616, title: "Medley Samba", artist: "Box Medley", duration: "10:10" },
  { id: 617, title: "Medley Rock Brasil", artist: "Box Medley", duration: "9:40" },
  { id: 618, title: "Medley Axé", artist: "Box Medley", duration: "9:00" },
  { id: 619, title: "Medley MPB", artist: "Box Medley", duration: "8:55" },
  { id: 620, title: "Medley Pop", artist: "Box Medley", duration: "10:05" },
];

const leoMagalhaesSongs = [
  { id: 621, title: "Larga de Bobeira", artist: "Léo Magalhães", duration: "3:15" },
  { id: 622, title: "Esquema Preferido", artist: "Léo Magalhães", duration: "3:22" },
  { id: 623, title: "Pode Me Ligar", artist: "Léo Magalhães", duration: "3:18" },
  { id: 624, title: "Galera da Pesada", artist: "Léo Magalhães", duration: "3:40" },
  { id: 625, title: "Você é Louca", artist: "Léo Magalhães", duration: "3:25" },
  { id: 626, title: "Paixão de Verão", artist: "Léo Magalhães", duration: "3:30" },
  { id: 627, title: "Faz Férias com a Gente", artist: "Léo Magalhães", duration: "3:12" },
  { id: 628, title: "Léo Magalhães", artist: "Léo Magalhães", duration: "3:20" },
  { id: 629, title: "Mexe o Bumbum", artist: "Léo Magalhães", duration: "3:35" },
  { id: 630, title: "Sinal de Saudade", artist: "Léo Magalhães", duration: "3:27" },
];

const striveToBe = [
    { id: 631, title: "If you Believe you", artist: "Strive to Be, Patch Crowe", duration: ""},
    { id: 632, title: "No apoligies", artist: "Strive to Be, Darlin Hatch", duration: "2:36"},
    { id: 633, title: "Trust in the Lord", artist: "Strive to Be, Lizzy Newbold", duration: "3:35"},
    { id: 634, title: "Found me", artist: "Strive to Be, Patch Crowe", duration: "3:16"},
    { id: 635, title: "Look unto Christ", artist: "Strive to Be, Halo Ray", duration: "3:57"},
    { id: 636, title: "Gather me", artist: "Strive to Be, Luz Ysabelle Cuevas", duration: "3:03"},
    { id: 637, title: "Disciple of Christ", artist: "Strive to Be, Benjamin Josiah", duration: "3:01"},
    { id: 638, title: "Fear Not", artist: "Strive to Be, Paige Anne", duration: "3:11"},
    { id: 639, title: "I can do all things", artist: "Strive to Be, Jarica Jamison", duration: "3:41"},
    { id: 640, title: "Faith", artist: "Strive to Be, Ben Olsen", duration: "3:04"}
];

export default{
    musicasTheWeeknd,
    musicasTravisScott,
    musicasTaylorSwift,
    musicasSZA,
    musicasShawnMendes,
    musicasSeuJorge,
    musicasSeanPaul,
    musicasRomeoSantos,
    queenSongs,
    musicasPostMalone,
    musicasPitbull,
    musicasOneRepublic,
    musicasOneDirection,
    michaelJacksonSongs,
    menudoSongs,
    mcCabelinhoSongs,
    matueSongs,
    lionelRichieSongs,
    lanaDelReySongs,
    katyPerrySongs,
    justinBieberSongs,
    jorgeMateusSongs,
    jBalvinSongs,
    izaSongs,
    imagineDragonsSongs,
    harryStylesSongs,
    nattanzinhoSongs,
    gusttavoLimaSongs,
    brunoMarsSongs,
    fiftyCentSongs,
    abbaSongs,
    acdcSongs,
    akonSongs,
    anittaSongs,
    arianaGrandeSongs,
    badBunnySongs,
    beyonceSongs,
    billieEilishSongs,
    britneySpearsSongs,
    cardiBSongs,
    coldplaySongs,
    chrisBrownSongs,
    djavanSongs,
    dojaCatSongs,
    drDreSongs,
    drakeSongs,
    duaLipaSongs,
    edSheeranSongs,
    eltonJohnSongs,
    eminemSongs,
    henriqueJulianoSongs,
    luanSantanaSongs,
    matheusKauanSongs,
    mcTutoSongs,
    maroon5Songs,
    rihannaSongs,
    luisFonsiSongs,
    blackpinkSongs,
    menosEMaisSongs,
    joseRicoMilionarioSongs,
    boxMedleySongs,
    leoMagalhaesSongs,
    striveToBe
};
